...
-- ─────────────────────────────────────────────────────────────────────
-- 4. PERSONAS Y TRABAJADORES (4 usuarios del sistema)
-- ─────────────────────────────────────────────────────────────────────
...
SELECT 'AC', p.persid, j.jobid, '2020-01-01',
       'christian.ortiz', md5('Admin2026!'), 'christian.ortiz@neoplus.pe'
FROM public.person p, public.job j
WHERE p.document = '45821036' AND j.name = 'Gerente General'
ON CONFLICT DO NOTHING;
...
FROM public.person p, public.job j
WHERE p.document = '46392018' AND j.name = 'Jefe de Mantenimiento'
ON CONFLICT DO NOTHING;
...
FROM public.worker w WHERE w.username = 'christian.ortiz'
ON CONFLICT DO NOTHING;
...
WHERE z.name = 'Arequipa Metropolitana' AND w.username = 'christian.ortiz'
ON CONFLICT DO NOTHING;
...
WHERE tp.name = 'Persona Jurídica'
  AND cs.name = 'Activo'
  AND cc.name = 'Habida'
  AND w.username = 'christian.ortiz'
ON CONFLICT DO NOTHING;
...
-- Insertar los 12 vehículos (igual que versión anterior)
DO $$
DECLARE
    v_workid  integer;
    v_coinid  smallint;
    ...
    i integer;
BEGIN
    SELECT workid INTO v_workid FROM public.worker WHERE username = 'christian.ortiz';
...
BEGIN
    ...
    SELECT cowoid INTO v_cowoid FROM company.worker
        ...
        WHERE w.username = 'christian.ortiz' LIMIT 1;
...
-- ─────────────────────────────────────────────────────────────────────
-- 9. MATERIALES E INVENTARIO (igual a versión anterior)
-- ─────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    INSERT INTO maintenance.action_catalog (altoid, name, category, recommended_quantity, unit_of_measure, useful_life_km, expires_by_time, useful_life_days) VALUES
        ...
        (v_l1,'Filtro de Aceite',             'Filtro',   '1',  'Unidad', 5000,  false, NULL),
        (v_l1,'Filtro de Aire',               'Filtro',   '1',  'Unidad', 15000, false, NULL),
        (v_l1,'Filtro de Combustible',        'Filtro',   '1',  'Unidad', 20000, false, NULL),
        (v_l1,'Líquido de Frenos DOT 4',      'Fluido',   '0.5','Litros', NULL,  true,  730),
        (v_l1,'Refrigerante',                 'Fluido',   '1',  'Litros', NULL,  true,  730),
        (v_l1,'Correa de Distribución',       'Repuesto', '1',  'Unidad', 60000, false, NULL),
        (v_l1,'Pastillas de Freno Del.',      'Repuesto', '1',  'Par',    30000, false, NULL),
        (v_l1,'Bujías de Encendido',          'Repuesto', '4',  'Unidad', 20000, false, NULL),
        (v_l2,'Inspección visual general',    'Inspección',  NULL, NULL, NULL, false, NULL),
        (v_l2,'Revisión de nivel de fluidos', 'Inspección',  NULL, NULL, NULL, false, NULL),
        (v_l2,'Inspección sistema de frenos', 'Inspección',  NULL, NULL, NULL, false, NULL),
        (v_l2,'Inspección de neumáticos',     'Inspección',  NULL, NULL, NULL, false, NULL),
        (v_l2,'Prueba post-servicio',         'Verificación',NULL, NULL, NULL, false, NULL)
    ON CONFLICT DO NOTHING;
END;
$$;
